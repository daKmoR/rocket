...
const {
  ...
  babelConfigSystemJs,
} = require('./babel/babel-configs.cjs');
const { bundledBabelHelpers } = require('./babel/rollup-plugin-bundled-babel-helpers.cjs');
const { isFalsy, pluginWithOptions, dedupedBabelPlugin } = require('./utils.cjs');

const resolve = resolvePkg.default;
...
function createBasicConfig(userOptions = {}) {
  ...
  const config = {
    ...
    plugins: [
      ...
      // minify js code
      !developmentMode && pluginWithOptions(terser, opts.terser),
    ].filter(isFalsy),
  };
